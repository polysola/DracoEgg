...
const App: React.FC = () => {
  ...
  const [notifications, setNotifications] = useState<NotificationItem[]>(() => {
    const saved = localStorage.getItem("arcdraco_notifications");
    if (saved) {
      try {
    ...
    return [
      {
        id: "notif-launch",
        type: "launch",
        title: "🌐 ARC Network Mainnet (Sept 16)",
        message: "ArcDraco launches on ARC Mainnet with confirmed USDC backing! Join Whitelist now.",
        timestamp: now - 300000, // 5 mins ago
        unread: true,
        actionText: "Join Whitelist",
      },
      {
        id: "notif-[#1]",
        type: "reward",
  ...
  useEffect(() => {
    localStorage.setItem("arcdraco_notifications", JSON.stringify(notifications));
  }, [notifications]);
  ...
  const handleConnectWalletAddress = (addr: string) => {
    ...
    if (user?.username) {
      ...
      saveUserWallet(user.username, addr);
    }
  };
  ...
  const renderHeader = () => (
    <div className="space-y-2 z-10 w-full">
      {/* Brand Header Bar with ArcDraco Logo Text */}
      <div className="flex items-center justify-between px-1">
        <div className="h-6 flex items-center">
          <img src={arcLogoText} alt="ArcDraco" className="h-full object-contain filter drop-shadow-[0_0_8px_rgba(168,85,247,0.5)]" />
        </div>
        <div className="flex items-center space-x-1.5">
          <span className="w-2 h-2 rounded-full bg-[#8b5cf6] animate-pulse"></span>
          <span className="text-[9px] font-black uppercase text-[#c084fc] tracking-widest">ARC NETWORK</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-1.5">
        ...
        <div className="flex items-center space-x-2 min-w-0">
          ...
          >
            <div className="p-0.5 rounded-full bg-gradient-to-r from-[#8b5cf6] via-[#a855f7] to-[#00d2ff] shadow-[0_0_12px_rgba(168,85,247,0.5)]">
              <img src={avatarSrc} alt="Avatar" className="w-9 h-9 sm:w-10 sm:h-10 rounded-full border-2 border-[#070514] object-cover bg-[#8b5cf6]/10" />
            </div>
            <div className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-[#00d2ff] border-2 border-[#070514]"></div>
          </div>
          <div className="shrink-0 cursor-pointer" onClick={() => setActiveModal("profile")}>
            <p className="text-xs sm:text-sm font-black text-white tracking-tight truncate max-w-[90px] sm:max-w-[130px] leading-tight">{user?.displayName || user?.username || "Arc Champion"}</p>
            <div className="flex items-center space-x-1 mt-0.5">
              <span className="text-[9px]">🛡️</span>
              <span className="text-[9px] sm:text-[10px] font-black text-[#c084fc] uppercase tracking-wider">{levelNames[levelIndex]}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-1 shrink-0">
          ...
          >
            {isWalletConnected ? (
              <>
                ...
                <span className="font-mono text-[#00d2ff] text-[10px] sm:text-[11px]">{truncatedWallet}</span>
              </>
            ) : (
              <>
                <span>💳</span>
          ...
          <button
            ...
            className="p-1.5 rounded-xl bg-[#0f0b24] hover:bg-[#8b5cf6]/20 hover:scale-105 transition-all border border-[#8b5cf6]/40 text-[#c084fc] shrink-0 relative"
            title="System Notifications Center"
          >
            <span className="text-xs sm:text-sm leading-none block">🔔</span>
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-red-500 rounded-full border border-[#070514] animate-ping"></span>
            )}
          </button>

          {/* X / Twitter Social Button */}
          <button
            onClick={() => window.open("https://x.com/ArcDracoEgg", "_blank")}
            className="p-1.5 rounded-xl bg-[#0f0b24] hover:bg-[#8b5cf6]/20 hover:scale-105 transition-all border border-[#8b5cf6]/40 text-[#c084fc] shrink-0"
            title="Follow ArcDraco on X"
          >
            <XIcon size={13} />
          </button>
          ...
          <button
            ...
            className="p-1.5 rounded-xl bg-[#0f0b24] hover:bg-[#8b5cf6]/15 transition-all border border-[#8b5cf6]/30 shrink-0"
            title="Settings"
          >
            <Settings size={13} className="text-[#c084fc]" />
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-2 pt-0.5">
        ...
        <div className="flex-1 min-w-0 pr-1">
          <div className="flex justify-between items-center text-[9px] font-bold mb-0.5">
            <span className="text-[#c084fc] font-black">Lv {levelIndex + 1} Progress</span>
            <span className="text-[8px] text-gray-400 font-mono">
              {Math.floor(points).toLocaleString()} / {levelMinPoints[levelIndex + 1] ? levelMinPoints[levelIndex + 1].toLocaleString() : "1000"} XP
            </span>
          </div>
          <div className="h-1.5 w-full bg-[#070514] rounded-full overflow-hidden p-[1px] border border-[#a855f7]/30">
            <div
        ...
        <div
          ...
          className="bg-[#0f0b24] px-2 py-1 rounded-xl border border-[#ffe600]/40 flex items-center space-x-1.5 cursor-pointer hover:border-[#8b5cf6]/50 transition-all shrink-0"
        >
      ...
      <div className="bg-[#0f0b24]/90 backdrop-blur-md p-3 rounded-2xl border border-[#00d2ff]/40 shadow-[0_0_25px_rgba(0,210,255,0.15)] flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-[#00d2ff]/20 border border-[#00d2ff] flex items-center justify-center shadow-[0_0_12px_#00d2ff]">
            <ETHIcon size={20} />
          </div>
          <div>
            <p className="text-[9px] text-[#00d2ff] uppercase tracking-widest font-black">ETH BALANCE</p>
            <p className="text-xl sm:text-2xl font-black text-white tracking-tight">{Math.floor(points).toLocaleString()}</p>
          </div>
        </div>
        <div className="bg-[#8b5cf6]/15 border border-[#8b5cf6]/40 text-[#c084fc] text-[10px] font-black px-2.5 py-1 rounded-xl shadow-[0_0_10px_rgba(139,92,246,0.2)]">
  ...
  return (
    <div className="flex justify-center h-screen max-h-screen relative overflow-hidden bg-[#060a12] font-orbitron">
        ...
        <div className="w-full text-[#f0eeff] h-screen max-h-screen font-bold flex flex-col justify-between max-w-md mx-auto relative z-10 overflow-hidden p-3 pb-24">
          {activeTab === "main" ? (
            <div className="flex-1 flex flex-col justify-between overflow-y-auto space-y-3">
              ...
              <div className="flex-1 flex flex-col items-center justify-center relative my-auto py-1">
                ...
                <div className="z-20 text-center mt-1">
                  <p className="text-base font-black text-[#c084fc] tracking-wider uppercase neon-purple-glow leading-none">TAP CORE</p>
                  <p className="text-[10px] text-gray-400 font-bold mt-0.5">Tap to mine ArcDraco</p>
                </div>
            ...
            <div className="flex-1 flex flex-col overflow-y-auto">
              {renderHeader()}
              <div className="flex-grow mt-3 bg-gradient-to-b from-transparent via-[#070514]/90 to-[#070514] rounded-t-[30px] relative top-glow-premium border-t border-[#8b5cf6]/30 overflow-hidden">
            ...
            <div className="flex-1 flex flex-col overflow-y-auto">
              <div className="px-4 z-10 pt-4 flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <div className="p-1 rounded-full bg-gradient-to-tr from-[#8b5cf6] via-[#a855f7] to-[#00d2ff] shadow-[0_0_12px_rgba(168,85,247,0.4)]">
                    <img src={avatarSrc} alt="Avatar" className="w-10 h-10 rounded-full object-cover border-2 border-[#070514] bg-[#8b5cf6]/10 p-0.5" />
                  </div>
                  <div>
                    <p className="text-base font-black text-[#f0eeff] tracking-tight leading-none">{user?.displayName || user?.username || "Arc Champion"}</p>
                    <p className="text-[9px] text-[#c084fc] font-black uppercase mt-1 tracking-wider neon-purple-glow">ArcDraco Cyber Master</p>
                  </div>
                </div>
              </div>
              <div className="flex-grow mt-4 bg-gradient-to-b from-transparent via-[#070514]/90 to-[#070514] rounded-t-[30px] relative top-glow-premium border-t border-[#070514] overflow-hidden">
                <div className="px-3 pt-4 flex-1 overflow-auto relative">
                  {isReferralLoading ? (
                    <div className="flex justify-center items-center h-40"><div className="w-8 h-8 border-4 border-[#8b5cf6]/20 border-t-[#8b5cf6] rounded-full animate-spin"></div></div>
                  ) : (
      ...
      {clicks.map((click) => (
        <div key={click.id} className="fixed text-4xl sm:text-5xl font-black text-[#c084fc] pointer-events-none neon-purple-glow z-[100] italic" style={{ top: `${click.y - 40}px`, left: `${click.x - 20}px`, animation: `float-up-fast 1s cubic-bezier(0.18, 0.89, 0.32, 1.28) forwards` }}>+{click.amount}</div>
      ))}
    </div>
  );
};
