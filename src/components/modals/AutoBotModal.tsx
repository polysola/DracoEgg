...
const AutoBotModal: React.FC<AutoBotModalProps> = ({
  ...
  const handleCollectEarnings = () => {
    ...
    onCollect();
    toast.success(`🤖 Claimed +${botEarnings.toLocaleString()} Real-time Offline Mining DRACO Coins!`);
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in font-orbitron">
      <div className="w-full max-w-md bg-[#0a0720] border border-[#8b5cf6]/40 rounded-[28px] p-6 text-[#f0eeff] shadow-[0_0_50px_rgba(139,92,246,0.3)] relative max-h-[90vh] overflow-y-auto">
        ...
        {/* Bot Info Panel */}
        <div className="bg-[#070514] p-4 rounded-2xl border border-[#8b5cf6]/30 mb-4 text-center space-y-2">
          <div className="w-14 h-14 rounded-full bg-[#8b5cf6]/20 border border-[#8b5cf6] flex items-center justify-center mx-auto text-3xl shadow-[0_0_20px_rgba(139,92,246,0.5)]">
            🤖
          </div>
          <div>
            <p className="text-[10px] text-gray-400 uppercase tracking-widest font-bold">Unclaimed Offline Mining Coins</p>
            <p className="text-3xl font-black text-[#c084fc] neon-purple-glow mt-1">
          ...
          <div className="flex justify-center items-center space-x-2 text-[10px] text-gray-400">
            ...
            <span>Mining Rate: +1,200 DRACO Coins / Hour</span>
          </div>
        </div>
        ...
        <div className="bg-[#070514] p-4 rounded-2xl border border-[#00d2ff]/30 mb-4 space-y-2 text-xs">
          ...
          <ul className="text-gray-300 space-y-1.5 text-[11px] list-disc list-inside">
            <li>Your AI Mining Bot auto-accumulates DRACO points continuously while you are offline.</li>
