...
const MintNftModal: React.FC<MintNftModalProps> = ({
  ...
  const storageKey = username ? `arcdraco_nft_minted_${username}` : "arcdraco_nft_minted_default";
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 bg-black/85 backdrop-blur-md animate-fade-in font-orbitron">
      <div className="w-full max-w-md bg-[#0a0f1d] border border-[#ffe600]/40 rounded-[28px] p-5 text-[#f0eeff] shadow-[0_0_60px_rgba(255,230,0,0.3)] relative max-h-[92vh] overflow-y-auto">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              <div className="flex items-center space-x-1.5">
                <h3 className="text-lg sm:text-xl font-black uppercase tracking-tight text-[#c084fc] leading-none">
                  ArcDraco Free NFT Mint
                </h3>
                <span className="text-[8px] font-black bg-[#8b5cf6]/20 text-[#c084fc] px-1.5 py-0.5 rounded border border-[#8b5cf6]/40">
                  DEMO
                </span>
              </div>
              <span className="text-[9px] text-[#00d2ff] font-black tracking-widest uppercase">
                ARC NETWORK TESTNET DEMO
              </span>
            </div>
          </div>
          <button
        ...
        <div className="bg-[#8b5cf6]/10 border border-[#8b5cf6]/30 rounded-2xl p-2.5 mb-3 flex items-center space-x-2 text-[10px]">
          <span className="text-base shrink-0">🧪</span>
          <p className="text-gray-300 font-medium leading-snug">
            <strong className="text-[#c084fc]">TESTNET DEMO MODE:</strong> Free NFT minting demonstration. Genesis Pass will be eligible for OpenSea Mint upon ARC Mainnet launch (Sept 16)!
          </p>
        </div>

        {/* NFT Hologram Showcase Card */}
        <div className="bg-gradient-to-b from-[#181138] to-[#070514] p-4 rounded-3xl border border-[#a855f7]/30 mb-4 text-center relative overflow-hidden group shadow-[0_0_30px_rgba(168,85,247,0.2)]">
          <div className="absolute top-2 left-2 bg-[#8b5cf6] text-white text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider shadow-[0_0_8px_#8b5cf6]">
            TESTNET DEMO
          </div>
          <div className="absolute top-2 right-2 bg-[#00d2ff] text-black text-[9px] font-black px-2.5 py-0.5 rounded-full uppercase tracking-wider shadow-[0_0_10px_#00d2ff]">
            LEGENDARY 0.01%
          </div>

          <div className="my-2 relative pt-2">
            <div className="w-32 h-32 mx-auto rounded-full bg-gradient-to-tr from-[#8b5cf6]/30 via-[#00d2ff]/20 to-[#ec4899]/20 flex items-center justify-center p-2 shadow-[0_0_40px_rgba(168,85,247,0.4)] group-hover:scale-105 transition-transform duration-500">
              <img
                src={hatchedEgg}
                alt="Cyber Genesis NFT Demo"
                className="w-28 h-28 object-contain drop-shadow-[0_10px_25px_rgba(168,85,247,0.5)] animate-pulse"
              />
            </div>
          </div>

          <h4 className="text-base font-black text-white uppercase tracking-wider mt-2">
            ArcDraco Cyber Genesis Pass
          </h4>
        ...
        {isMinted ? (
          ...
          <button
            ...
            disabled={isMinting}
            className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#8b5cf6] via-[#a855f7] to-[#00d2ff] text-white font-black text-xs sm:text-sm uppercase tracking-wider shadow-[0_0_30px_rgba(168,85,247,0.5)] hover:scale-[1.02] transition-all flex items-center justify-center space-x-2"
          >
            {isMinting ? (
              <>
